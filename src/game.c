...
gint game_destroy_lines(gboolean count_score) {
    ...
    if (have_del) {
        if (pref_get_show_destroy()) {
            ...
            animarray = g_new0(
            struct gtkb_animarray, rules_get_width() * rules_get_height() * gtkbTheme->maxdestphases);
            for (y = 0; y < rules_get_height(); y++) {
                for (x = 0; x < rules_get_width(); x++) {
            ...
            for (animtime = 0, i = 0; i < animcadres;) {
                ...
                } while ((tve.tv_sec - tvs.tv_sec) * 1000000 + tve.tv_usec - tvs.tv_usec <
                         (animarray[i].time - animtime) * 1000);
