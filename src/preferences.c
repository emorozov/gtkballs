...
void preferences_dialog(void) {
    ...
    buttons[PR_SHOW_NEXT] = ut_check_button_new(_("Show colors that will appear on next turn"),
                                                pref_get_show_next_colors(), vbox);
    ...
    buttons[PR_SHOW_ANIM] = ut_check_button_new(_("Show animation of disappearing of the ball"),
                                                pref_get_show_destroy(), vbox);
    ...
    buttons[PR_TIME_VALUE] = ut_spin_button_start_new(_("Time limit (seconds): "), 1, 3600,
                                                      timer_get_limit() > 0 ? timer_get_limit() : 60, vbox);
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(theme_scrolled_window), GTK_POLICY_AUTOMATIC,
    ...
    gtk_tree_selection_set_mode(gtk_tree_view_get_selection(GTK_TREE_VIEW(buttons[PR_THEME_LIST])),
